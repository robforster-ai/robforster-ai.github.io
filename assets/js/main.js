document.addEventListener('DOMContentLoaded', function () {
    ...
    var revealObserver = new IntersectionObserver(function (entries) {
      entries.forEach(function (entry) {
        if (entry.isIntersecting) {
          ...
          entry.target.classList.add('visible'); /* compat */
  ...
  if (overlay) {
    overlay.querySelectorAll('a').forEach(function (link) { link.addEventListener('click', closeMenu); });
  }
  ...
  navLinks.forEach(function (link) {
    ...
    if (!href) return;
    if (href === currentPath || (currentPath !== '/' && href !== '/' && currentPath.startsWith(href.split('#')[0]))) {
  ...
  /* Homepage use-case chip filter (new design) */
  var homeChips = document.querySelectorAll('[data-chip-biz]');
  var homeUcCards = document.querySelectorAll('[data-uc-biz]');
  if (homeChips.length) {
    homeChips.forEach(function (chip) {
      chip.addEventListener('click', function () {
        homeChips.forEach(function (c) { c.classList.remove('active'); });
        chip.classList.add('active');
        var val = chip.getAttribute('data-chip-biz');
        homeUcCards.forEach(function (card) {
          var biz = card.getAttribute('data-uc-biz');
          card.style.display = (val === 'All' || biz === val) ? '' : 'none';
        });
      });
    });
  }
  ...
  (function () {
    ...
    var msg = '★ WELCOME TO ROB FORSTER\'S HOME PAGE ★ ENTERPRISE AI IS MY PASSION ★ Best viewed in Netscape Navigator 4.0 at 800×600 ★ Please sign my GUESTBOOK ★ Last updated: TODAY ★ NO HOTLINKING OR I WILL FIND YOU ★ Under construction since 1996 ★ Please enable JavaScript AND Flash Player ★ AI is totally RADICAL ★ This page is 100% Y2K compliant ★    ';
    ...
    if (navEl && navEl.parentNode) navEl.parentNode.insertBefore(marqueeWrap, navEl.nextSibling);

    var construction = document.createElement('div');
    construction.className = 'geocities-construction';
    construction.innerHTML = '&#x1F6A7;&nbsp; THIS SITE IS UNDER CONSTRUCTION &nbsp;&#x1F6A7;<br><span style="font-size:0.8rem;color:#000000;font-weight:normal;">Please come back when it\'s finished. (It\'s never finished.)</span>';
    var heroEl = document.querySelector('.hero, .hero-feature');
    if (heroEl && heroEl.nextSibling) heroEl.parentNode.insertBefore(construction, heroEl.nextSibling);

    var counter = document.createElement('div');
    counter.className = 'geocities-counter';
    counter.innerHTML = '&#x1F441; You are visitor number <strong>#001,337</strong>';
    var footerGrid = document.querySelector('.footer-grid, .footer-inner');
    if (footerGrid) footerGrid.parentNode.insertBefore(counter, footerGrid.nextSibling);
    ...
    function spawnSparkle(x, y) {
      ...
      setTimeout(function () { el.style.opacity = '0'; el.style.transform = 'scale(0) translateY(-' + (12 + Math.random() * 18) + 'px)'; }, 30);
